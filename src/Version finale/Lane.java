...
public class Lane {
    ...
    public void enleveVoitures(){
        Iterator<Car> c = this.cars.iterator();
        while(c.hasNext()){
            if(c.next().enleveVoiture()){
                c.remove();
            }
        }
    }
    ...
    public void addGraphics(){
        if(this.density==0){
            BufferedImage image=this.game.images.Safe;
            for(int i=0;i<game.width;i++){
                game.getGraphic().add(new Element(i, this.ord - this.game.Score(), image));
            }
        }
    }

    public boolean enleveLigne(){
        return this.ord<this.game.Score()-5;
    }
